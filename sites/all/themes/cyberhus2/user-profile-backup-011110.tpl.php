...
<div class="profile">
  ...
  $loggedin_user_access = in_array('authenticated user',$user->roles);
  $cyberaktive_access = (in_array('Cyberaktiv',$user->roles) || in_array('Cyberaktiv +18',$account->roles));
  $moderator_access = (in_array('Koordinator',$user->roles) || in_array('Vejleder',$account->roles));
  ?>
</div>

<?php 
/* the following roles have a special profile layout with 3 columns */ 
...
<div id="profile-wrapper" class="clear-block">
  <div id="profile-left">
	...
	<div id="profile-left-lower">
		<?php if ($moderator_access): ?>
			<div id="profile-moderation">
			
			<?php /*only the koordinator should see this headline*/ if (in_array('Koordinator',$account->roles)):?>
				<h3><?php print t('Comments for moderation: ');?></h3> 
			<?php endif; ?>  		
	  		<?php
	          $viewname = 'seneste_debatindlaeg';
	          $viewresultat = views_embed_view($viewname, $display_id = 'block_2');
	          print $viewresultat;
			?> 
